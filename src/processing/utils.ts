...
export function byteArrayToInt(uint8Array: Uint8Array): number {
  return (uint8Array[0] << 24) | (uint8Array[1] << 16) | (uint8Array[2] << 8) | uint8Array[3];
}

export async function jpegRoundTripBgr32f(cv: any, bgr32f: any, quality = 0.95, unitRange = true) {
  // --- ENCODE ---
  ...
  const blob: any = await new Promise(res => encCanvas.toBlob(res, 'image/jpeg', quality));
...
export async function getJpegSubsampling(file: File) {
      ...
      const label = (() => {
        ...
        switch (key) {
          ...
          case '1x2-1x1-1x1':
            return '4:4:0';
          default:
            return `non-canonical (Y ${Y.H}x${Y.V}, Cb ${Cb.H}x${Cb.V}, Cr ${Cr.H}x${Cr.V})`;
        }
      })();
...
export async function decodeJpeg(cv: any, jpegBytes: Uint8Array) {
...
export async function matToJpegFileResult(mat: any, filename = 'image.jpg', quality = 0.9): Promise<FileResult> {
  ...
  const blob: Blob = await new Promise(resolve => canvas.toBlob(resolve, 'image/jpeg', quality));
