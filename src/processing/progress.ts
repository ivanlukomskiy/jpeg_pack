...
export class ProgressTracker {
  ...
  public markInProgress(step: number) {
    ...
    this.status.set(step, {
      code: StepStatusCode.IN_PROGRESS,
      startTime: Date.now(),
    });
  }
  public markCurrentStepCompleted() {
    const currentStep = Array.from(this.status.entries()).find(
      ([, status]) => status.code === StepStatusCode.IN_PROGRESS,
    );
    if (currentStep) {
  ...
  public markCurrentStepFailed(error: string) {
    ...
    if (currentStep) {
      ...
      state.error = error;
      console.error(
        `${this.describe(currentStep[0])} failed with error in ${state.endTime! - state.startTime!} ms: ${error}`,
      );
...
export function createEncodingProgressTracker(progress?: Record<number, StepStatus>) {
  ...
  const progressMap = progress
    ? new Map<number, StepStatus>(Object.entries(progress).map(([k, v]) => [parseInt(k), v]))
    : undefined;
