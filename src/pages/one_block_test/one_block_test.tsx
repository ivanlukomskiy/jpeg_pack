    ...
    <Flex direction={'column'} gap={'xl'}>
      <Flex direction={'row'} gap={'xl'}>
        <Flex direction={'column'} gap={'sm'} style={{ alignItems: 'left', width: '200px' }}>
          ...
          {inpType == INP_TYPE_RANDOM && (
            <NumberInput label="Size" min={0} hideControls value={randInputSize} onChange={onRandInputSizeChanged} />
          )}
          ...
          <Button onClick={randomizeAndCheck}>Randomize & check</Button>
        </Flex>
        <Flex direction={'column'} gap={'sm'}>
          <Title size={'lg'}>Result</Title>
          <MatRender mat={res} />
          {res && <Button onClick={() => decode(res)}>Decode</Button>}
        </Flex>
        <Flex direction={'column'} gap={'sm'}>
